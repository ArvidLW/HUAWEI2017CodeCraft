...
//            }
//        }
//        if(minFTmp>Graph::netNode[routeArr[n-1] ]->mRequire){
//            minFTmp=Graph::netNode[routeArr[n-1] ]->mRequire;
//        }
//        //减去
//        for(int i=0;i<n-1;++i){
//            Graph::gNet[routeArr[i+1] ][routeArr[i] ]->rCapacity=Graph::gNet[routeArr[i+1] ][routeArr[i] ]->rCapacity - minFTmp;
//        }
//        Graph::netNode[routeArr[n-1] ]->mRequire=Graph::netNode[routeArr[n-1] ]->mRequire-minFTmp;
//        //p->mCapacity=p->mCapacity+minFTmp;
//
//        //Graph::gNet[t][routeArr[n-1]]
//        //倒数第二位上记录消费节点
...
//    //printf("node: %d->%d\n",p->node0,p->node1);
//    //printf("pmCap: %d<-%d,%.f , %.f , %.f\n",p->node1,p->node0,p->mCapacity,p->rCapacity,Graph::gNet[p->node1][p->node0]->rCapacity);
////    if(p->node1==t){
////        printf("cap: %d -> %d ,%.f",p->node0,p->node1,Graph::gNet[p->node1][p->node0]->rCapacity);
////    }
